...
const MedicineForm = ({ onSuccess, medicine }) => {
  ...
  const validateMedicineName = (name) => {
    ...
    timeout = setTimeout(async () => {
      ...
      } catch { setMedicineValid(false); }
      finally { setCheckingMedicine(false); }
    }, 500);
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!medicineValid) { showError("Please enter a valid medicine name."); return; }
    ...
    try {
      ...
      setFormData({ medicineName: "", dosage: "", frequency: "daily", time: [""], startDate: "", endDate: "" });
      setIsEditing(false);
  ...
  return (
    ...
    >
      <div className="border-b border-blue-100 bg-gradient-to-r from-blue-500 to-indigo-600 px-8 py-6 text-white">
        ...
        <p className="mt-1 text-sm text-blue-100">AI-powered medication management for better health outcomes</p>
      </div>

      <div className="space-y-6 p-8">

        {/* ✅ Inline banners instead of alert() */}
        {successMsg && (
          <div className="flex items-center gap-2 rounded-xl bg-green-50 border-2 border-green-200 px-4 py-3 text-sm font-bold text-green-700">
            ✅ {successMsg}
          </div>
        )}
        {errorMsg && (
          <div className="flex items-center gap-2 rounded-xl bg-red-50 border-2 border-red-200 px-4 py-3 text-sm font-bold text-red-700">
            ⚠️ {errorMsg}
          </div>
        )}

        <div className="group">
          <label className="mb-2 block text-sm font-bold text-gray-800">Medicine Name <span className="text-red-500">*</span></label>
          <div className="relative">
            <input type="text" name="medicineName" placeholder="e.g., Aspirin, Metformin, Lisinopril"
              ...
              className={`w-full rounded-xl border-2 px-4 py-3.5 font-medium shadow-sm transition duration-200 focus:outline-none focus:ring-4 ${
                medicineValid ? "border-blue-200 bg-white focus:border-blue-400 focus:ring-blue-100"
        ...
        <div>
          <label className="mb-2 block text-sm font-bold text-gray-800">Dosage <span className="text-red-500">*</span></label>
          <input type="text" name="dosage" placeholder="e.g., 500mg, 2 tablets, 5ml"
        ...
        <div>
          <label className="mb-2 block text-sm font-bold text-gray-800">Frequency <span className="text-red-500">*</span></label>
          <div className="relative">
        ...
        <div className="space-y-3">
          <label className="block text-sm font-bold text-gray-800">⏰ Reminder Time(s) <span className="text-red-500">*</span></label>
          <div className="space-y-2">
            {formData.time.map((t, index) => (
              <div key={index} className="group flex items-center gap-3 transition-all duration-200">
                <div className="relative flex-1">
                  <input type="time" value={t} onChange={(e) => handleTimeChange(index, e.target.value)}
                    className="w-full rounded-xl border-2 border-blue-200 bg-white px-4 py-3.5 font-medium shadow-sm transition duration-200 focus:border-blue-400 focus:outline-none focus:ring-4 focus:ring-blue-100"
                    required
                  />
                  <div className="pointer-events-none absolute inset-y-0 right-3 flex items-center text-blue-600">
        ...
        <div className="rounded-2xl border-2 border-blue-100 bg-blue-50/30 p-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="mb-2 block text-xs font-bold uppercase tracking-wide text-gray-600">Start Date <span className="text-red-500">*</span></label>
            ...
            <div>
              <label className="mb-2 block text-xs font-bold uppercase tracking-wide text-gray-600">End Date <span className="text-xs font-normal text-gray-400">(Optional)</span></label>
        ...
        <button type="submit" disabled={loading || !medicineValid}
        ...
        >
          <span className="relative z-10 flex items-center justify-center gap-2 text-base">
            {loading ? (
              <><span className="h-5 w-5 animate-spin rounded-full border-2 border-white border-t-transparent"></span><span>Saving...</span></>
            ) : isEditing ? (
              <><svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg><span>Update Medicine</span></>
            ) : (
              <><svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg><span>Save Medicine & Set Reminders</span></>
            )}
          </span>
          {!loading && medicineValid && (
