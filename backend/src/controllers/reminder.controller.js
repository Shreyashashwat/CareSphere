...
const handleMissedReminder = async (reminderId) => {
  ...
  const delay = (new Date(reminder.userResponseTime) - scheduledTime) / 60000;

  let risk = 0.5;
  try {
    risk = await predictAdherenceRisk(scheduledTime.getHours(), scheduledTime.getDay(), delay);
  } catch (e) {}

  await createPreReminderIfHighRisk(reminder, risk);

  let newTime =
    risk > 0.75 ? new Date(scheduledTime.getTime() + 30 * 60000)
    : risk > 0.5 ? new Date(scheduledTime.getTime() + 15 * 60000)
  ...
  const safeRisk = typeof risk === "number" && !Number.isNaN(risk)
    ? Math.min(Math.max(risk, 0), 1) : 0.5;

  await AIAnalytics.findOneAndUpdate(
    { userId: reminder.userId, medicineId: reminder.medicineId },
    { riskLevel: safeRisk, lastAnalysis: new Date() },
    { upsert: true }
  );

  await applyPreReminderToFutureDoses(reminder.userId, reminder.medicineId, risk);

  try {
    const calendarData = await Calendar.findOne({ userId: reminder.userId });
    if (calendarData) {
      await updateMedicineInGoogleCalendar(reminder, calendarData, newTime, "Rescheduled ⏰ (AI-adjusted)");
    }
  } catch (calErr) {
    console.warn("⚠️ Calendar reschedule skipped:", calErr.message);
  }
};
...
const learnAndShiftHabit = async (userId, medicineId) => {
  const recent = await Reminder.find({
    userId, medicineId, status: "taken", userResponseTime: { $ne: null },
  }).sort({ userResponseTime: -1 }).limit(3);
