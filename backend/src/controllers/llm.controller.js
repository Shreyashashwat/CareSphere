...
`;
  ...
  parsed.insights = parsed.insights.map((insight) => {
    ...
    // Rule 1: purely positive content must not be high
    if (isPositive && !isNegative && insight.priority === "high") {
      insight.priority = "low";
    }

    // Rule 2: negative content must never be low
    if (isNegative && insight.priority === "low") {
      insight.priority = "medium";
    }

    // Rule 3: ≥50% miss ratio → force high
  ...
  // Cap at max 2 per priority — keep first 2 of each, discard extras
  ...
  // If Groq produced no "low" insights, synthesise one from the trend/streak data
